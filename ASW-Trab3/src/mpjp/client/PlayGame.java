...
public class PlayGame extends Composite {
	...
	private static final int TOTAL_ANIMATION_FRAMES = (int) (ANIMATION_TIME_IN_MILLISECS * FRAMES_PER_SEC / 1000L);
	 ...
	 * @param workspaceId
	 */
	PlayGame(final DeckPanel panels, final PuzzleServiceAsync managerService, PuzzleInfo puzzleInfo,
			String workspaceId) {
		if (canvas == null) {
								...
								try {
									puzzleServiceAsync.getCurrentLayout(workspaceId, new AsyncCallback<PuzzleLayout>() {
										...
										public void onSuccess(PuzzleLayout result) {
											...
											if (workspaceId != null && currentPuzzleLayout != null
	...
	/**
	 * Pool data (needed on in concurrent solving) and paints the puzzle. This will
	 * update the elapsed time on the footer even if the person is idle.
	 */
	...
	public void mouseReleased(MouseUpEvent e) {
		...
		if (selectedId != null) {
			if (withinWorkspace(e.getX(), e.getY())) {
				...
				try {
					puzzleServiceAsync.connect(workspaceId, selectedId, newMousePosition,
	...
	/**
	 * Checks if event is within this workspace. When dragging, events with
	 * coordinates outside the window can be sent to it.
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Animate the complete puzzle (without the pieces borders) from current
	 * position until filling the complete workspace
	 */
	...
	/**
	 * Paint all the blocks, leaving the selected block for last to make it hover
	 * over the others
	 * 
	 * @param puzzleLayout PuzzleLayout puzzleLayout
	 */
	...
	/**
	 * Convert a PieceShape into a {@code java.asw.Shape}, useful for displaying
	 * using AWT and use its features
	 * 
	 * @param pieceShape
	 * @return
	 */
	private void paintShape(PieceShape pieceShape) {
		...
		for (Segment segment : pieceShape.getSegments()) {
			if (segment instanceof LineTo) {
				...
				gc.quadraticCurveTo(controlPoint.getX(), controlPoint.getY(), endPoint.getX(), endPoint.getY());
				...
				gc.bezierCurveTo(controlPoint1.getX(), controlPoint1.getY(), controlPoint2.getX(), controlPoint2.getY(),
