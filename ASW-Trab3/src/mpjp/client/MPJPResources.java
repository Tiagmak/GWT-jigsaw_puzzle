 ...
 * {@code /mpjp/resource/ } but may be changed using 
 * {@link #setResourceDir(String)}.
 * 
 * All methods in this class are static. The {@link #loadAudio(String)}
 * method returns an audio object that may be played using it as context
 * for the {@code play()} method as in the following example.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The {@link #loadImageElement(String, Consumer)} method is void. It
 * receives as argument a consumer of a {@code ImageElement} that is
 ...
 * </pre>
 * 
 * This {@code ImageElement} can be
 * given as argument to the {@code Context2d.drawImage()} method. 
 * 
...
public class MPJPResources {
	 ...
	 * This pathname should correspond to the prefix configured in 
	 * {@code WEB-INF/web.xml} with the {@code <url-pattern>}
	 * element, contained in the {@code <servlet-mapping>} element.
	 *  
	...
	/**
	 * Change the pathname of the remote directory holding the resources 
	 * fetch using HTTP. This pathname should correspond to the prefix configured in 
	 * {@code WEB-INF/web.xml} with the {@code <url-pattern>}
	 * element, contained in the {@code <servlet-mapping>} element.
	...
	/**
	 * An audio resource implemented as a native JavaScript Audio element
	 * GWT doesn't have audio support and needs to be implemented as
	 * native (JavaScript) method calls. Audio resources are played
	 * by invoking their {@code play()} method
	 */
	...
	/**
	 * Load an audio resource. The returned instance can be used for playing
	 * the audio clip
	 * <pre>
	...
	/**
	 * Load given image and execute consumer when loaded. Locally available
	 * images are reused, otherwise are loaded from the server. A typical use
	 * is
	 * <pre>
	 * 		MPJPResources.loadImageElement(imageName, i -&gt; {
	 * 				image = i;
	 ...
	 */
	public static void loadImageElement(String imageName,
	...
	/**
	 * Show object on the browser's console. Used mostly for reporting errors
	 * that cannot be thrown as exceptions.
	 * 
